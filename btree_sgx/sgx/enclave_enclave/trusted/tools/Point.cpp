...
{
	if (m_dimension != p.m_dimension) throw IllegalArgumentException("Point::operator ==: Points have different number of dimensions.");
...
// IObject interface
//
Tools::Geometry::Point* Tools::Geometry::Point::clone() throw (Tools::NotSupportedException)
{
...
unsigned long Tools::Geometry::Point::loadFromByteArray(byte* const data)
{
	byte* ptr = data;
...
void Tools::Geometry::Point::storeToByteArray(unsigned long& len, byte** data)
{
...
{
	if (m_dimension != p.m_dimension) throw IllegalArgumentException("Point::getMinimumDistance: Shapes have different number of dimensions.");
...
double Tools::Geometry::Point::getCoordinate(unsigned long index) const throw (IndexOutOfBoundsException)
{
