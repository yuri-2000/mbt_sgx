...
std::vector<Node*> Graph::findresult(unsigned int lowId, unsigned int highId, unsigned int k) {
	vector<Node*> resultVector;
	if (k == 0)
		return resultVector;
	...
	for (int i = lowId; i <= highId; i++) {
		if (this->getTree() != NULL) { // the b tree is built, so we could find the node using the tree
			sourceNode.push_back(this->getTree()->search(i));
		}
		else { // we must traverse the "nodes" vector in the graph to find the node
			for (int i = 0; i != this->numberOfNodes(); i++) {
				if (this->getNode(i)->getNodeId() == i) {
					sourceNode.push_back(this->getNode(i));
				}
			}
		}
	...
	for (int i = 0; i < sourceNode.size(); i++) {
		minHeap->push(sourceNode[i]);
		sourceNode[i]->setState(Node::INHEAP);
	}
	while (minHeap->empty() == false) {
		Node* current = minHeap->pop();
		if (PoIsFound >= k && current->weight() > lastWeight)
